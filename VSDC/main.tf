terraform {
  backend "azurerm" {
    ...
    #resource_group_name  = "RSG-SAP-DEV"
    #storage_account_name = "azudevstgstfstate"
    #container_name       = "tfstate"
    #key                  = "tfstatebj1"
    key                  = "vocera-dev-vsdc.tfstate"
     }
}
...
resource "azurerm_virtual_machine_extension" "example" {
  ...
  settings = <<SETTINGS
 {
  "commandToExecute": "/usr/bin/bash /root/tmscript.sh"
 }
SETTINGS
}
...
resource "azurerm_network_interface" "NW" {
  ...
  ip_configuration {
    name                          = "Primary"
    subnet_id                     = substr(each.value.ServerType,0,2)=="DB"?var.DBSubnetid:var.AppSubnetid
